...
def get_color_terminal(word, pattern):
    # Colors: Green #538d4e, Yellow #b59f3b, Gray #3a3a3c
    bg_green = "\033[48;2;83;141;78m"
    bg_yellow = "\033[48;2;181;159;59m"
    bg_gray = "\033[48;2;58;58;60m"
    # Fix from Screenshot 2026-05-02 at 12.58.37 AM.png: Bold Bright White
...
def generate_tree_pdf(start_word, results_dict):
    ...
    # Priority sorting: Grey (_) > Yellow (y) > Green (g)
    p_map = {'_': 0, 'y': 1, 'g': 2}
    ...
    for secret in pool:
        ...
        while s_t <= 6:
            ...
            if len(s_p) == 1:
                res_t = s_t + 1
                if res_t > 6:
                    missed.append(secret); total_turns += 7; stats[6] += 1
                else:
                    total_turns += res_t; stats[res_t - 1] += 1
                break
            s_t += 1
...
def run_game(mode, hard, limit, start_word, target=None):
    ...
    return turn, solve_path_colored, solve_path_plain, full_history
...
def main():
    ...
    if mode == 1:
        ...
        for i, t in enumerate(targets):
            ...
            print(f"Avg: {total_turns / (i + 1):.4f}")

        if mode in [2, 4]:
            with open(f"{start_w.upper()}.txt", "w") as f: f.write("\n".join(txt_out))
            generate_tree_pdf(start_w, pdf_data)
            print(f"\nWin Rate: {sum(dist[:6]) / len(targets) * 100:.2f}%")
